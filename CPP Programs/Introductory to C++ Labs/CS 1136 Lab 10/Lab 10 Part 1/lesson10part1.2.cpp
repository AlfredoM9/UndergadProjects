...
// Function prototypes
bool readData(char answers[], const int NUM_QUESTIONS,string fileName, ifstream & inputFile );
void displayResults(char correctAnswers[], char studentAnswers[], const int ANSWERS);

int main()
{
   // Create ifstream variables
   ifstream inputFileStudent, inputFileAdmin;
   ...
   successOrNot = readData(studentAnswers, ANSWERS, studentFile, inputFileStudent);

   // If successfully read: continue
   if (successOrNot)
   {
      // Call readData again but with a different file for a different array
      successOrNot = readData(correctAnswers, ANSWERS, adminFile, inputFileAdmin);
