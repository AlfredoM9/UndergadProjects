...
void displayData(int numOfSpools, int numOfSpoolsInStock, double shippingAndHandling)
{
   ...
   // If the number of ordered spools is greater than number of spools in stock
   if (numOfSpools > numOfSpoolsInStock)
   {
      // Calculate number of spools on the back order
      ...
      // Display the amount ready to be shipped
      cout << "Spools ready to ship: " << numOfSpoolsInStock << endl;

   } // end if (numOfSpools > numOfSpoolsInStock)

   // Else if the number of spools ordered is smaller than the spools in stock
   else
   {
      // The num of spools on the back order becomes zero
      numOfSpoolsOnBackOrder = 0;

      // Calculate shipping and handling costs
      shippingAndHandling *= numOfSpools;

      // Calculate subtotal
      subtotalCharges = numOfSpools * 100.0;

      // Display the amount ready to be shipped
      cout << "Spools ready to ship: " << numOfSpools << endl;
