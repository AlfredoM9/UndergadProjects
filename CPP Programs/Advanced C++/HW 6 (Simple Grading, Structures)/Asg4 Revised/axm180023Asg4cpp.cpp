...
int displayMenu(Student* stuList[], int numStudents);
...
int main()
{
   ...
   do
   {
      userChoice = displayMenu(stuList, numStudents);
...
void sortInfo(Student* stuList[], int numStudents, int userChoice)
{
   if (userChoice == 1)
   {
      ...
      for (int count = 0; count < numStudents; count++)
      {
         ...
         cout << right << setw(10) << (*stuList[count]).studentID
            << right << setw(20) << (*stuList[count]).studentName
