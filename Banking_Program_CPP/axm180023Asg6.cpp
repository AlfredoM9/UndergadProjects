...
Description: This program will serve as a menu for a bank. The menu will consist of  *
			 of several options. The first option will allow for the creation of a   *
...
int displayMenu()
{
	...
	do
	{
		...
		cout << "7. Add interest to Savings Account" << endl;
...
void createSavings(Bank *accounts[], const int MAX_ACCTS, int &numOfAccts)
{
	...
	else
	{
		...
		// Variable for user's choice and set flag
		int userOption;
		invalid = true;

		do
		{
			// Ask if the user wants to deposit now
			cout << "Would you like to deposit now?" << endl;
			cout << "1. Yes" << endl;
			cout << "2. No" << endl;
			...
			if (cin >> userOption)
			{
				// If yes, call the deposit function
				if (userOption == 1)
				{
					deposit(accounts, numOfAccts);
					invalid = false;
				...
				// Else if no then just set flag to false
				else if (userOption == 2)
					invalid = false;

				// Otherwise invalid entry display message
				else
					cout << "An invalid entry has been entered. Please try again" << endl;
			} // end if

			// If not an int display error message
			else
			{
				cout << "An invalid entry has been entered. Please try again" << endl;
				cin.clear();
...
void createAccount(Bank *accounts[], const int MAX_ACCTS, int &numOfAccts)
{
	...
	{
		int userOption;
		bool invalid = true, infoRequest;
		cout << "You are about to create a bank account." << endl;

		// Ask if additional info wants to be entered
		do
		{
			cout << "Would you like to provide information about the account you are about to create?" << endl;
			cout << "1. Yes" << endl;
			cout << "2. No" << endl;
			// If an int is entered
			if (cin >> userOption)
			{
				...
				// Else if no then just create a bank account
				else if (userOption == 2)
				{
					invalid = false;
					infoRequest = false;
					accounts[numOfAccts] = new Bank;
					numOfAccts++;
					cout << "Just keep in mind this account will have no name and a default account number of 9999" << endl << endl;
				} // end else
				// Otherwise invalid entry display message
				else
					cout << "An invalid entry has been entered. Please try again" << endl;
		...
		} while (invalid); // while invalid input is entered.
...
void displayAccountInfo(Bank *accounts[], const int numOfAccts)
{
	...
	else
	{
		...
		// Counter;
		int counter = 0;
		// Display all counts type nothing
		for (int count = 0; count < numOfAccts; count++)
		{
			if (accounts[count]->getAccountNum() == 9999)
			{
				counter++;
				// Display header once
				if(counter == 1)
					cout << left << setw(15) << "Account#" << left << setw(15) << "Name" << left << setw(15) << "Balance" << left << setw(15) << "Account Type" << endl;
				cout << left << setw(15) << accounts[count]->getAccountNum() << left << setw(15) << accounts[count]->getName() << left << "$" << setw(14) << fixed << setprecision(2) << accounts[count]->getBalance()
					<< left << setw(15) << "No type" << endl;
			} // end if
		}// end for loop
...
void addInterest(Bank *accounts[], const int numOfAccts)
{	
	// Check if they are any savings account
	bool savingAccount = false;
	for (int count = 0; count < numOfAccts && !savingAccount; count++)
	{
		if (accounts[count]->getAcctType() == 'S')
		savingAccount = true;
	} // end for loop
	...
	if (numOfAccts == 0 || !savingAccount)
		cout << "There are no saving accounts saved." << endl;
		...
		do
		{
			...
			if (cin >> tempAcctNum)
			{
				...
				for (int count = 0; count < numOfAccts; count++)
				{
					...
					else if ((count + 1) == numOfAccts && controlMessage)
						cout << "No account was found with such account number. Please try again" << endl;
				} // end for loop
			}
